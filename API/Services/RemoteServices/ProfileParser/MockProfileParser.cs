...
namespace OuchRBot.API.Services.RemoteServices.ProfileParser
{
    public class MockProfileParser : IProfileParser
    {
        ...
        public Task<ReadOnlyCollection<Intership>> GetInternshipsAsync(Stream resumeStream)
        {
            ...
            {
                new Intership("http://rtuitlab.dev#nut", "Собиратель орехов"),
                new Intership("http://rtuitlab.dev#front", "Frontend разработчик"),
                new Intership("http://rtuitlab.dev#back", "Backdoor разработчик"),
                new Intership("http://rtuitlab.dev#ml", "ML разработчик"),
            };
            int n = list.Count;
            var rnd = new Random();
            while (n > 1)
